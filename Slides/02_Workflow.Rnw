...
load(file.path('data', "NHANES_for_lectures.RData"))
...
There are several packages in \textsf{R} that provide functions to visualize
incomplete data.\\\bigskip

Examples are:\\
\textbf{naniar}, \textbf{VIM},  \textbf{visdat}, \textbf{mice}, \textbf{JointAI},
\textbf{Amelia}, \ldots\\[3ex]
\end{frame}
...
@
\includegraphics[width = \linewidth]{figure/plot_all.pdf}
\end{frame}
...
head(mdp[, -c(8:14)]) # omit some columns to fit it on the slide
tail(mdp[, -c(8:14)])
@
...
\begin{frame}[fragile]{\thesection. \insertsection}
\framesubtitle{\thesection.\thesubsection. \insertsubsection}
<<JointAI_md_pattern, eval = FALSE>>=
JointAI::md_pattern(NHANES)
@
\begin{center}
\includegraphics[width = 0.85\linewidth]{figure/JointAI_mdpat.pdf}
\end{center}
\end{frame}
...
@
\centering\includegraphics[width = 0.95\linewidth]{figure/VIM_mdpat.pdf}
\end{frame}

\begin{frame}[fragile]{\thesection. \insertsection}
\framesubtitle{\thesection.\thesubsection. \insertsubsection}
<<naniar_vis_miss, eval = FALSE>>=
naniar::vis_miss(NHANES)
@
\begin{center}
\includegraphics[width = \linewidth]{figure/naniar_vis_miss.pdf}
\end{center}
\end{frame}

\begin{frame}[fragile]{\thesection. \insertsection}
\framesubtitle{\thesection.\thesubsection. \insertsubsection}
<<naniar_miss_upset, eval = FALSE>>=
naniar::gg_miss_upset(NHANES[, 1:10], nsets = 10)
@
\begin{center}
\includegraphics[width = \linewidth]{figure/naniar_miss_upset.pdf}
\end{center}
\end{frame}
...
<<corrplot syntax, eval = FALSE, size = 'small'>>=
...
\framesubtitle{\thesection.\thesubsection. \insertsubsection}
\begin{center}
\includegraphics[width = \textheight]{figure/corrplot.pdf}
\end{center}
\end{frame}
...
Good auxiliary variables \cite{Buuren2012}
\begin{itemize}
...
values of \Rstring{hgt} and \Rstring{wgt}.
\end{frame}
...
@
Here we have
\Sexpr{sum(is.na(NHANES$hgt) & !is.na(NHANES$wgt))} +
\Sexpr{sum(is.na(NHANES$wgt) & !is.na(NHANES$hgt))} =
\Sexpr{sum(xor(is.na(NHANES$hgt), is.na(NHANES$wgt)))} cases in which
either \Rstring{hgt} or \Rstring{wgt} is observed.\bigskip

We would like to impute \Rstring{hgt} and \Rstring{wgt} separately and
...
@
To \blue{prevent feedback} from \Rstring{BMI} in the imputation of \Rstring{hgt}
and \Rstring{wgt} the \Rarg{predictorMatrix} needs to be modified.
\end{frame}

\begin{frame}[fragile]{\thesection. \insertsection}
\framesubtitle{\thesection.\thesubsection. \insertsubsection}

Since \Rstring{BMI} depends on \Rstring{wgt}, and the two variables are highly correlated
($\rho = $\Sexpr{round(cor(NHANES$wgt, NHANES$BMI, use = "pair"), 2)})
...
imputations for \Rstring{hgt} and \Rstring{wgt} are drawn, the resulting
values of \Rstring{BMI}, will match \Rstring{hgt} and \Rstring{wgt} from
...
To practice the content of the previous section 
find the \textbf{html version} of the practical here:\\
\begin{block}{}
...
\framesubtitle{\thesection.\thesubsection. \insertsubsection}
\includegraphics[width = \linewidth]{figure/convergence_imp2.pdf}
\end{frame}
...
\begin{frame}[fragile]{\thesection. \insertsection}
\framesubtitle{\thesection.\thesubsection. \insertsubsection}
<<densityplot_imp2, eval = FALSE>>=
densityplot(imp2)
@
\includegraphics[width = \linewidth]{figure/densityplot_imp2.pdf}
\end{frame}
...
imputed values of \Rstring{hgt} are smaller than the observed values.
\end{frame}
...
\begin{center}
<<densityplot2, eval = FALSE>>=
densityplot(imp2, ~hgt|gender, plot.points = TRUE)
@
\includegraphics[width = 0.8\linewidth]{figure/densityplot_imp2_hgt.pdf}
\end{center}
\end{frame}
...
% 
% \pause
% However, there are not enough missing values of \Rstring{hgt} per
...
\begin{frame}[fragile]{\thesection. \insertsection}
\framesubtitle{\thesection.\thesubsection. \insertsubsection}
For (combinations of) variables with very few missing values a
\Rfct{stripplot} may be better suited. Here we can also split
the data for \Rstring{gender} and \Rstring{race}.
<<stripplot1, eval = FALSE>>=
stripplot(imp2, hgt ~ race|gender, pch = c(1, 20),
          scales = list(x = list(rot = 45)))
@
\includegraphics[width = \linewidth]{figure/stripplot_imp2.pdf}
\end{frame}
...
source(file.path(projdir, "Slides", "Rscripts", "propplot.R"))
@
\end{frame}
...
@
\includegraphics[width = \linewidth]{figure/propplot_imp2.pdf}
\end{frame}
...
To practice the content of the previous section you may find the \textbf{html version} of the practical here:\\
\begin{block}{}
...
load(file.path("workspaces/imp2b.RData"))
imp2combi <- ibind(imp2, imp2b)
@
