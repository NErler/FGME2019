...
# source(file.path(projdir, "Slides", "Rscripts/plots_PartIII.R"))
...
\begin{column}{0.54\linewidth}
\only<handout>{
\centering\includegrahics[width = \linewidth]{figure/plong1_1b.pdf}
}
\only<beamer>{
\only<1>{
\centering\includegraphics[width = \linewidth]{figure/plong1_1.pdf}
...
\centering\includegraphics[width = 0.9\linewidth]{figure/p_compsurv.pdf}
...
To practice imputation with non-linear forms or interaction terms find the \textbf{html version} of the practical here:\\
\begin{block}{}
...
To practice imputation with longitudinal data find the \textbf{html version} of the practical here:
\begin{block}{}
...
\begin{frame}[fragile]{\thesection. \insertsection}
\framesubtitle{\thesection.\thesubsection. \insertsubsection}
White et al. \cite{White2009} found that when \blue{covariate effects and
cumulative incidences are rather small},
...
If covariate effects $\beta_x$ and $\beta_z$ are small, \blue{$H_0(t)\approx H(t)$},
...
<<mice_surv01, size = 'small', eval = FALSE>>=
...
<<mice_surv02, size = 'small', eval = FALSE>>=
# setup run
...
the imputation:
<<mice_surv03, size = 'small', eval = FALSE>>=
...
\begin{frame}[fragile]{\thesection. \insertsection}
\framesubtitle{\thesection.\thesubsection. \insertsubsection}
\includegraphics[width = \linewidth]{figure/p_comp_surv.pdf}
...
To practice imputation with survival data find the \textbf{html version} of the practical here:
\begin{block}{}
