...
load("../data/NHANES3_for_practicals.RData")

source("../data/propplot.R")
load("../data/imps_nonlin.RData")
...
### Dataset 
For this practical, we will use the **NHANES3** data, another subset of the 
data we have already seen in the lecture slides and the previous practicals.
It contains only those cases that have
...
`NHANES3_for_practicals.RData` on your computer.
If you know the path to the file, you can also use `load("<path>/NHANES3_for_practicals.RData")`.
...
```{r eval = static, echo = FALSE, fig.width = 8, fig.height=6}
...
savedimps_impJAV <- mice(NHANES3, method = meth, predictorMatrix = pred,
                         maxit = 10, m = 5)

savedimps_impPAS <- mice(NHANES3, method = methPAS, predictorMatrix = pred,
                         maxit = 10, m = 5)

savedimps_JointAI <- lm_imp(wgt ~ gender + bili + age * (chol + HDL) + hgt, data = NHANES3,
                            auxvars = ~ race + SBP + hypten + WC,
                            models = c(bili = 'lognorm'), refcats = 'largest',
                            n.iter = 100, seed = 2019)

savedimps_lm1 <- lm_imp(wgt ~ gender + bili + age * (chol + HDL) + hgt, data = NHANES3,
                        n.iter = 100, seed = 2019)


savedimps_lm2 <- lm_imp(wgt ~ gender + bili + age * (chol + HDL) + hgt, data = NHANES3,
                        auxvars = ~ race + SBP + hypten + WC,
                        models = c(bili = 'lognorm'), refcats = 'largest',
                        n.iter = 100, seed = 2019)

savedimps_lm3 <- lm_imp(wgt ~ gender + bili + age * (chol + HDL) + hgt, data = NHANES3,
                        auxvars = ~ race + SBP + hypten + WC,
                        models = c(bili = 'lognorm'), refcats = 'largest',
...
Specify the linear regression model using the function `lm_imp()` with the model
formula given at the beginning of this practical
...
JointAIargs <- rbind(
  ...
  c("`auxvars`", paste0("one-sided formula of variables that are not part of the analysis",
...
plot(MC_error(lm2))
```
The plot is just shows the ratio `MCSE/SD`. It is usually faster to check
if all dots are left of the vertical 5% line than to read all numers in the 
table.
For our final results, we should increase the number of iterations (either by 
inceasing `n.iter` or `n.chains`) to get a more precise estimate of the posterior
distribution.
...
```{r JointAIlm3-solution, eval = FALSE}
